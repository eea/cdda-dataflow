...
(:
 : Module Name: CDDA 2017 - Designation boundaries (Main module)
 :
 : Version:     $Id$
 ...
 :)
(:~
 : XQuery script for CDDA 2017 - Designation boundaries table.
...
(:
 TEST files:
 declare variable $source_url as xs:string external;
 declare variable $source_url := "../test/cdda_desigboundaries-2017.xml";
 :)
(:~ DD table ID :)
declare variable $xmlconv:SCHEMA_ID as xs:string :=  $rules:DESIGBOUND_SCHEMA;
(:~ DD table elements namespace prefix :)
declare variable $xmlconv:ELEM_SCHEMA_NS_PREFIX as xs:string :=  $rules:DESIGBOUND_NS_PREFIX;
...
as element(div)
{
    let $container := "designation_boundaries"
    let $ruleResults := (
        ...
        , xmlutil:executeDuplicatesCheck($url, $xmlconv:SCHEMA_ID, $xmlconv:ELEM_SCHEMA_NS_PREFIX, $xmlconv:KEY_ELEMENT, "2",
            ("ISO3", "DESIG_ABBR"), false(), $container)
