...
(:
 : Module Name: CDDA 2017 - National overview (Main module)
 :
 : Version:     $Id$
 ...
 :)
(:~
 : XQuery script for CDDA 2017 - National overview table.
 ...
 : XML Schema:  http://dd.eionet.europa.eu/GetSchema?id=TBL9119
 :
 :)
...
(:
 TEST files:
 declare variable $source_url as xs:string external;
 declare variable $source_url := "../test/cdda_nationaloverview-2014.xml";
 :)
(:~ DD table ID :)
declare variable $xmlconv:SCHEMA_ID as xs:string :=  $rules:NATIONALOVERVIEW_SCHEMA;
(:~ DD table elements namespace prefix :)
declare variable $xmlconv:ELEM_SCHEMA_NS_PREFIX as xs:string :=  $rules:NATIONALOVERVIEW_NS_PREFIX;
(:~ key element identifying row in the table, used in in the output key="" attribute:)
declare variable $xmlconv:KEY_ELEMENT as xs:string := fn:concat($xmlconv:ELEM_SCHEMA_NS_PREFIX, "ISO3");
...
as element(div)
{
    let $container := "national_overview"
    let $ruleResults := (
        ...
        , xmlutil:executeDuplicatesCheck($url, $xmlconv:SCHEMA_ID, $xmlconv:ELEM_SCHEMA_NS_PREFIX, $xmlconv:KEY_ELEMENT, "2",
            ("PARENT_ISO", "ISO3", "Category", "Major_ecosystem"), false(), $container)
